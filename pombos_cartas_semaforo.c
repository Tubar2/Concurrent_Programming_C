...
int main(int argc, char **argv) {
    ...
    sem_unlink("mochila");
    mochila = sem_open("mochila", O_CREAT, 0777, 20);
    if (mochila == SEM_FAILED) {
        fprintf(stderr, "%s\n", "ERROR creating semaphore mochila");
        exit(EXIT_FAILURE);
    }
    sem_unlink("pombo");
    pombo = sem_open("pombo", O_CREAT, 0777, 0);
    if (pombo == SEM_FAILED) {
        fprintf(stderr, "%s\n", "ERROR creating semaphore pombo");
        exit(EXIT_FAILURE);
    }
